...
public class SimplePhotoComponentUI extends PhotoComponentUI
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (!JPhotoComponent.getModel().isFlipped())
	...
	private void drawNotes(Graphics2D g, JPhotoComponent c) {
		for (PhotoNote note : c.getModel().getNotes())
			drawNote(g, c, note);
	}
	...
	private void drawNoteBorders(Graphics2D g, JPhotoComponent c, PhotoNote note, ArrayList<String> lines) {
		...
		g.setColor(c.getModel().isCurrentNote(note) ? Color.green : Color.lightGray);
		
		int x = note.position.x + position.x;
		int y = note.position.y + position.y;
		
		g.drawLine(x - metrics.getDescent(), y - metrics.getAscent(),
				x - metrics.getDescent() + 5, y - metrics.getAscent());
		g.drawLine(x - metrics.getDescent(), y - metrics.getAscent(),
				x - metrics.getDescent(), y - metrics.getAscent() + 5);
	...
	private void drawStrokes(Graphics2D g, JPhotoComponent c) {
		...
		for (PhotoStroke stroke : c.getModel().getStrokes()) {
	...
	private void drawStroke(Graphics2D g, JPhotoComponent c, Point pointA, Point pointB) {
		...
		g.drawLine(
			(int) pointA.x + position.x, (int) pointA.y + position.y,
			(int) pointB.x + position.x, (int) pointB.y + position.y
		);
	}
	...
	private void drawImage(Graphics2D g, JPhotoComponent c) {
		...
		g.drawImage(c.getModel().getImage(), position.x, position.y, null);
	}
	...
	private Point getImagePosition(JPhotoComponent c) {
		int w = c.getModel().getImage().getWidth(null);
		int h = c.getModel().getImage().getHeight(null);
	...
	private PhotoNote findNoteByPosition(JPhotoComponent c, int x, int y) {
		...
		ArrayList<PhotoNote> notes = c.getModel().getNotes();
		...
		for (PhotoNote note : notes) {
			...
			if (note.position.x - 5 <= x - imagePosition.x &&
				note.position.y - 5 <= y - imagePosition.y &&
	...
	public void mouseReleased(MouseEvent e) {
		if (((JPhotoComponent) e.getComponent()).getModel().isFlipped())
			isDrawing = false;
	}
    ...
    public void mousePressed(MouseEvent e) {
    	if (((JPhotoComponent) e.getComponent()).getModel().isFlipped()) {
    		isDrawing = true;
    		((JPhotoComponent) e.getComponent()).getModel().startStroke();
    	}
    }
          ...
          c.getModel().flip();
        	...
        	if (c.getModel().isFlipped()) {
        		...
        		if (selectedNote != null)
        			c.getModel().selectNote(selectedNote);
        		else {
        			Point notePosition = new Point(e.getPoint().x - imagePosition.x,
        					e.getPoint().y - imagePosition.y);
        			c.getModel().startNote(notePosition);
        		}
        	}
        }
    }
    ...
    public void mouseDragged(MouseEvent e) {
		...
		if (isDrawing) {
    		...
    		c.getModel().addPointToStroke(newPoint);
		}
	}
	...
	public void keyTyped(KeyEvent e) {
    	...
    	if (c.getModel().isFlipped()) {
    		if (e.getKeyChar() == KeyEvent.VK_BACK_SPACE)
    			c.getModel().backspaceCurrentNote();
    		else
    			c.getModel().addCharToCurrentNote(e.getKeyChar());
    	}
    }
	...
	public void keyPressed(KeyEvent e) {
    	...
    	if (c.getModel().isFlipped()) {
    		if (e.getModifiers() == 4 && e.getKeyCode() == 61)
    			c.getModel().increaseCurrentNoteSize();
    		else if (e.getModifiers() == 4 && e.getKeyCode() == 45)
    			c.getModel().decreaseCurrentNoteSize();
    	}
	}
